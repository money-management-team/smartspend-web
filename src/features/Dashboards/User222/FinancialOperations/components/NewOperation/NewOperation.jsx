...
import { ApiError, createIdempotencyKey } from "../../../api/apiClient";
...
export default function NewOperation({ accounts, categories, onCreated }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setMessage("تمت إضافة العملية بنجاح.");
      setHasError(false);
    ...
    <section className="new-operation">
      ...
      >
        ...
        <label className="new-operation__field">
          ...
          >
            {availableCategories.length === 0 && <option value="">لا توجد تصنيفات متاحة</option>}
        ...
        <label className="new-operation__field">
          ...
          <select
            ...
            required
          >
            {accounts.length === 0 && <option value="">لا توجد حسابات متاحة</option>}
